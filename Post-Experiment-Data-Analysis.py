...
class Experiment:
	...
	def smooth_no_ppb(self,somethin):
		...
		self.no_ppb_smooth = self.no_ppb.rolling(window=win,win_type='boxcar').mean()
		self.nox_ppb_smooth = self.nox_ppb.rolling(window=win,win_type='boxcar').mean()
	...
	def run(self, datafile, exp_times):
		...
		for row in listed_dataset:
			...
			if self.filter_dataset(row[1], exp_times) == True:
				...
				self.no_ppb.append(self.conv_no_v_to_ppb(row[2]))
				self.nox_ppb.append(self.conv_nox_v_to_ppb(row[3]))

		self.kinetics()

		'''for i,row in enumerate(self.duration):
			print row,'\t', self.no_ppb[i], '\t',self.nox_ppb[i]'''
		return self
